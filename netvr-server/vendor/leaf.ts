// @ts-nocheck
// modified from https://github.com/mandarineorg/leaf/blob/main/leafCompiler.ts
// to support import maps
// https://github.com/mandarineorg/leaf/issues/14
import { walkSync } from 'https://deno.land/std@0.85.0/fs/mod.ts'
import {
  fileSystemExecutable,
  fileSystemPropertyName,
} from 'https://deno.land/x/leaf@v1.0.4/constants.ts'
import { MEM_METHODS } from 'https://deno.land/x/leaf@v1.0.4/functions/methods.ts'
...
export class Leaf {
  ...
  private static storageToJson(): string {
    const storage: FileStorageNumbers = Object.fromEntries(
      Object.entries(this.files).map(([filePath, content]) => [
        filePath,
        [...content],
      ]),
    )
  ...
  public static async compile(options: CompileOptions) {
    ...
    options.contentFolders.forEach((folder) => {
      for (const entry of Array.from(walkSync(folder)).filter(
        (item) => item.isFile,
      )) {
    ...
    if (options && options.flags) {
      ...
      if (options.flags.indexOf('--unstable'))
        options.flags = options.flags.filter(
          (item) => item.toLowerCase() != '--unstable',
        )
      if (options.flags.indexOf('--allow-read'))
        options.flags = options.flags.filter(
          (item) => item.toLowerCase() != '--allow-read',
        )
